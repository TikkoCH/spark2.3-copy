...
private[spark] object Utils extends Logging {
  ...
  */
  ...
  /**
    * 首先尝试根据环境变量获取Spark_Conf_Dir
    * 最后会返回Spark默认属性文件的路径,通过JAVA代码读取环境变量,尝试获取spark-env.sh中的SPARK_CONF_DIR
    * 如果没有的话会根据SPARK_HOME/conf下的spark-defaults.conf文件.
    * Return the path of the default Spark properties file. */
  def getDefaultPropertiesFile(env: Map[String, String] = sys.env): String = {
    env.get("SPARK_CONF_DIR")
      .orElse(env.get("SPARK_HOME").map { t => s"$t${File.separator}conf" })//map结果是SPARK_HOME/conf
      .map { t => new File(s"$t${File.separator}spark-defaults.conf")}//尝试获取spark-defaults.conf
      .filter(_.isFile)
